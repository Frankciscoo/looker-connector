...
# Functions to gather user input using Streamlit
...
def gather_look_ids():
    ...
    for i in range(number_of_looks):
        ...
        globals()[f"look_{i}"] = look_id  # Dynamically create variables
...
st.write("Provide the configuration details for connecting Looker with Google Sheets.")

config_file = st.radio("Do you have a configuration file to upload?", ('Yes', 'No'))

if config_file == 'Yes':
    filename = st.text_input("Enter the path to the configuration file:")
...
if st.button('Show Configuration Summary'):
    ...
    st.write(f"**Gsheet Title:** {title}")
    st.write(f"**Tab Names:** {tab_names}")
    st.write(f"**Cell Range for Pasting Data:** {range_name}")

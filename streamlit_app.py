...
if not client_id or not client_secret:
    ...
    def check_filters_in_explores(explore_fields, all_filter, group_filters, group_filter_assignments, exclude_filters_assignment):
        ...
        def check_filter_against_explores(filter_name, explore_key, explore_value):
            ...
            if (filter_name not in dimensions and filter_name not in measures and filter_name not in parameters):
    ...
    # Streamlit UI
    def main():
        st.title("Looker Filter Checker")
    
        # Authenticate and mount drive
        st.write("Authenticating and mounting drive...")
        ...
        if client_id and client_secret and company_domain:
            auth_token = generate_auth_token(client_id, client_secret, company_domain)
            ...
            looks_list = st.text_area("Looks List (comma-separated)").split(",")
            looks = pd.DataFrame(np.array(looks_list).reshape(-1, 1), columns=['look_id'])
            
            # Dynamically get sheet tabs
            explore_fields = {}
            for i in range(number_of_looks):
                tab_name = st.text_input(f"Tab Name {i}", key=f"tab_name_{i}")
                if tab_name:
                    tab = sheet.worksheet(tab_name)
                    look_id = looks.loc[i, "look_id"]
                    model, view = get_model_view(look_id, url, headers)
                    dimensions, measures, parameters = get_fields_from_explore(model, view, url, headers)
                    explore_fields[i] = {'dimensions': dimensions, 'measures': measures, 'parameters': parameters}
            
            # Filters input
            all_filter = st.text_area("All Filters (JSON format)", height=200)
            group_filters = st.text_area("Group Filters (JSON format)", height=200)
            group_filter_assignments = st.text_area("Group Filter Assignments (JSON format)", height=200)
            exclude_filters_assignment = st.text_area("Exclude Filters Assignment (JSON format)", height=200)
            
            if st.button("Check Filters"):
                all_filter = pd.read_json(all_filter, orient='index').to_dict()
                group_filters = pd.read_json(group_filters, orient='index').to_dict()
                group_filter_assignments = pd.read_json(group_filter_assignments, orient='index').tolist()
                exclude_filters_assignment = pd.read_json(exclude_filters_assignment, orient='index').to_dict()
                
                result = check_filters_in_explores(
                    explore_fields, all_filter, group_filters, group_filter_assignments, exclude_filters_assignment
                )

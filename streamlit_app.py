...
if not client_id or not client_secret:
    ...
    # Initialize `looks` DataFrame from `looks_list`
    if number_of_looks and looks_list:
        looks = pd.DataFrame(looks_list, columns=["look_id"])

    # Initialize `group_filters` and assignments
    group_filters = {
    ...
    # Gather additional filter configurations
    ...
    # Button to perform filter checking
    if st.button('Check Filters'):
        explores = {}
        for i in range(number_of_looks):
            tab_name = tab_names[i]
            if tab_name:
                sheet = None  # Initialize the Google Sheets API client here
        ...
        result = check_filters_in_explores(explore_fields, all_filter, group_filters, [group_filter_0_assignment, group_filter_1_assignment, group_filter_2_assignment], exclude_filters_assignment)
        st.text(result)

...
# ---- STREAMLIT UI ----
st.title("🔗 Looker-GSheets Connector")
st.write("Connect Looker data with Google Sheets!")
...
if not client_id or not client_secret:
    st.info("Please enter Looker API credentials.", icon="🗝️")
    st.stop()  # Stops execution until credentials are provided

st.success("Credentials provided successfully!")

# ---- LOAD CONFIGURATION FILE ----
def load_config_from_file(file_content):
    global number_of_looks, looks_list, company_domain
    config = file_content.splitlines()
    config_dict = {}
    ...
    company_domain = config_dict.get('company_domain', '').strip()
    number_of_looks = int(config_dict.get('number_of_looks', '0'))
    looks_list = config_dict.get('looks_list', '').split(',')
...
def main():
    try:
        ...
        st.write(f"Visit this [link]({authorization_url}) to grant access:")
...
def generate_auth_token():
    try:
        ...
        response = requests.post(f'{company_domain}:19999/api/4.0/login', data=data)
        response.raise_for_status()
        return response.json().get('access_token')
    except requests.exceptions.RequestException as e:
        st.error(f"Error authenticating Looker: {e}")
        return None

HEADERS = {'Authorization': f'token {generate_auth_token()}'}

def get_model_view(look_id):
    try:
        response = requests.get(f"{company_domain}:19999/api/4.0/looks/{look_id}", headers=HEADERS)
        response.raise_for_status()
        data = response.json()
        return data.get('query', {}).get('model'), data.get('query', {}).get('view')
    except requests.exceptions.RequestException:
        return None, None

def get_fields_from_explore(model, explore):
    try:
        response = requests.get(f"{company_domain}:19999/api/4.0/lookml_models/{model}/explores/{explore}", headers=HEADERS)
        response.raise_for_status()
        data = response.json()
        return (
            [item.get('name') for item in data.get('fields', {}).get('dimensions', [])],
            [item.get('name') for item in data.get('fields', {}).get('measures', [])],
            [item.get('name') for item in data.get('fields', {}).get('parameters', [])]
        )
    except requests.exceptions.RequestException:
        return [], [], []
...
if st.button('Check Filters'):
    ...
    for look_id in looks_list:
        model, view = get_model_view(look_id)
        if model and view:
            dimensions, measures, parameters = get_fields_from_explore(model, view)
            explore_fields[look_id] = {'dimensions': dimensions, 'measures': measures, 'parameters': parameters}

    st.success("Filters validated successfully!")
